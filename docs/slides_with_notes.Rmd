...
*PLINK is already installed on midway2.* To use the previously
 installed PLINK, run:
...
download PLINK in the same place as the other workshop materials.

+ URL: www.cog-genomics.org/plink2

For example, on midway2, you may run these commands to download PLINK:

```{bash get-plink, eval=FALSE}
cd $SCRATCH/genetic-data-analysis-1
wget https://tinyurl.com/twseejly -O plink.zip
unzip plink.zip
./plink --version
```
...
+ Or download the data from EBI:
    ...
    + *Make sure to copy the file to same location as the other
      workshop materials.*
...
Make sure you can display a plot in your current R session.
...
Run this command to convert the genotypes on chromosome 1 from VCF to
the PLINK text format. This may take a few minutes.
...
format. Note that this step may take several minutes to complete.
...
Remove related samples (optional)
=================================
...
+ It is helpful to keep track of the data processing steps you took.
...
in PLINK can be used to create a matrix:
...
> Instructor notes: For those who haven't made it this far, they can
> get a copy of the "recoded" data from ~pcarbo/share/1kg_recoded.raw.gz.
...
particular case, we can set these missing entries to the *mean
genotype*:
...
Double-check that there are no more missing genotypes:
...
Use the "rpca" function from the rsvd package to compute the first 4
PCs---that is, the 4 components that explain the most variation in
the genotypes:

```{r run-rpca}
library(rsvd)
pca <- rpca(geno,k = 4,center = TRUE,
...
Before moving on, let's save our results:
...
+ We glossed over some details about encoding genotypes as a matrix.
