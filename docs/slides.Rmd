...
+ Columns in the `.map` file: (1) chromosome; (2) marker id; (3)
...
+ The PLINK text format is easy to inspect and manipulate with simple
shell commands (e.g., `wc`, `grep`, `cat`, `cut`, `paste`).

+ The PLINK binary format (`.bed`) is a much more efficient way to
store genotype data (but is not human readable).
