...
```{r load-data-table-pkg, eval=FALSE}
...
```{r missing-genotypes, eval=FALSE}
100*mean(is.na(geno))
```
...
```{r fill-missing-genotypes, eval=FALSE}
p <- ncol(geno)
for (j in 1:p) {
...
```{r check-missing-genotypes, eval=FALSE}
sum(is.na(geno))
```
